...
/**
 *  个人感觉：这个FragmentUtils感觉不是很有用
 add                   : 新增 fragment  FragmentUtils.add(getSupportFragmentManager(), mFragments, R.id.fragment_container, curIndex)
                                        FragmentUtils.add(getFragmentManager(),ChildFragment.newInstance(),R.id.child_fragment_container,false, true);
 show                  : 显示 fragment
 hide                  : 隐藏 fragment
 showHide              : 先显示后隐藏 fragment  FragmentUtils.showHide(index, mFragments)
 replace               : 替换 fragment
 pop                   : 出栈 fragment
 ...
 getTop                : 获取顶部 fragment      FragmentUtils.getSimpleName(FragmentUtils.getTop(getFragmentManager())
