...
1.给文字添加颜色和背景色
 new SpanUtils().append("删除线").setForegroundColor(Color.GREEN).setBackgroundColor(Color.LTGRAY).create()

2.给文字添加删除线
 new SpanUtils().append("删除线").setStrikethrough().create()

3.段落首行缩进两字，其他行不缩进
 new SpanUtils().append("测试段落缩，首行缩进两字，其他行不缩进").setLeadingMargin((int) tv_3.getTextSize() * 2, 10).create()

4.给文字右上角添加上标
 new SpanUtils().append("测试").appendLine("上标").setSuperscript().create()

5.设置字体
 new SpanUtils().append("monospace 字体").setFontFamily("monospace").create()

5.设置自定义字体
 new SpanUtils().append("自定义字体").setTypeface(Typeface.createFromAsset(getAssets(), "fonts/dnmbhs.ttf")).create()

6.给文字添加点击事件
 new SpanUtils().append("测试").appendLine("点击事件").setClickSpan(clickableSpan).create()
...
7.给文字添加链接
 new SpanUtils().append("测试").appendLine("Url").setUrl("https://github.com/Blankj/AndroidUtilCode").create()

8.设置文字居中对齐
 new SpanUtils().append("居中对齐").setAlign(Layout.Alignment.ALIGN_CENTER).create()
...
11.文字渐变
 new SpanUtils().appendLine("颜色渐变").setShader(new LinearGradient(0, 0,
 64 * density * 4, 0,
 getResources().getIntArray(R.array.rainbow),
 null,
 Shader.TileMode.REPEAT)).setFontSize(64, true).create()

12.文字+图片
 new SpanUtils().append("居中").setBackgroundColor(Color.GREEN)
 .appendImage(R.drawable.shape_spannable_block_low, SpanUtils.ALIGN_CENTER).create()

13.图片着色到文字上
 new SpanUtils().append("图片着色").setFontSize(64, true).setShader(new BitmapShader(BitmapFactory.decodeResource(getResources(), R.drawable.span_cheetah),
 Shader.TileMode.REPEAT,
 Shader.TileMode.REPEAT)).create()

14.文字添加空格
 new SpanUtils().append("测试空格").appendSpace(30, Color.LTGRAY).appendSpace(100).appendSpace(50, Color.GREEN).create()
